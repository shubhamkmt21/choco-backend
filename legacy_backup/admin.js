...
async function loadProducts() {
    ...
    try {
        ...
        // Handle both structure formats (direct array or { data: [] })
        ...
        products.forEach(p => {
            ...
            const safeImage = p.image || 'https://placehold.co/50x50?text=No+Img';

            tr.innerHTML = `
                <td><img src="${safeImage}" class="prod-thumb" onerror="this.src='https://placehold.co/50x50?text=No+Img'"></td>
                ...
                <td>
                    <div style="display: flex; gap: 5px;">
                        <button class="action-btn" onclick='editProduct(${JSON.stringify(p)})'><i class="fas fa-edit"></i></button>
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:red;">Failed to load products. Server might be down.</td></tr>';
    }
}

function editProduct(product) {
    // If product passed as object (from onclick), use it directly
    // If passed as ID (legacy), fetch it or find it. 
